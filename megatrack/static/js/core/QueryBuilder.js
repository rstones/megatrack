...
mgtrk.QueryBuilder = (function() {
    ...
    QueryBuilder.init = (_parent) => {
        ...
        // queries for a dataset-method combination
        queryBuilder.datasetMethodQueries = [];
        ...
        const buildQueryObject = function() {
            const newQuery = {};
            let queries = queryBuilder.datasetMethodQueries;
            for (let i=0; i<queries.length; i++) {
                if (Object.keys(queries[i].constraints).length > 0 && !queries[i].excluded) {
                    const datasetCode = queries[i].datasetCode;
                    const methodCode = queries[i].methodCode;
                    const queryKey = `${datasetCode}`;
                    newQuery[queryKey] = {
                                            "method": methodCode,
                                            "constraints": {}
                                        };
                    for (let key in queries[i].constraints) {
                        ...
                        newQuery[queryKey].constraints[key] = {"type": queries[i].constraints[key].type};
                        switch (newQuery[queryKey].constraints[key].type) {
                            case "radio":
                                newQuery[queryKey].constraints[key].value = constraint.queryRow.find(`input[name="${key}"]:checked`).val();
                                break;
                            case "range":
                                newQuery[queryKey].constraints[key].min = $(constraint.sliderDiv).slider('values',0);
                                newQuery[queryKey].constraints[key].max = $(constraint.sliderDiv).slider('values',1);
                                break;
                            case "checkbox":
                                ...
                                if (vals.length > 0) {
                                    newQuery[queryKey].constraints[key].values = vals;
                                } else {
                                    ...
                                    delete newQuery[queryKey].constraints[key];
                                }
                                break;
                        }
                    }
        ...
        /*
         * We store the datasets and methods logically server side (ie. a single 
         * dataset can have tractography generated by multiple methods) and join
         * them on the client side so users will choose to compare dataset-method
         * combinations (ie. BRC_ATAS-DTI, BRC_ATLAS-SD, etc)
         */
        const addDatasetMethodQuery = (tableId, datasetCode, methodCode, dataset) => {
            
            const datasetMethodQuery = {
            ...
            $(`#${tableId} > tbody`).append(`
                    <tr id="${datasetCode}-${methodCode}-query" class="dataset-query-row">
                        <td class="dataset-query-cell">
                            <div class="dataset-query-heading">${datasetMethodQuery.dataset[methodCode].name}</div>
                            <div class="dataset-query-constraint-select">
                                <select id="${datasetCode}-${methodCode}-query-select">
                                    <option value="default" disabled selected>Add constraint...</option>
                                </select>
                            </div>   
                            <div class="dataset-remove">
                                <div id="${datasetCode}-${methodCode}-remove" class="clickable remove-icon dataset-remove-icon" title="Remove dataset"></div>
                            </div>
                            <div class="clear"></div>
                            <table id="${datasetCode}-${methodCode}-query-constraints-table" class="query-constraints-table"><tbody></tbody></table>
                        </td>
                    </tr>
                    <tr id="${datasetCode}-${methodCode}-spacer" class="dataset-spacer-row"><td></td></tr>`);
            
            // populate query select menu from query builder data for this dataset
            for (let key in dataset.queryParams) {
                $(`#${datasetCode}-${methodCode}-query-select`).append(`<option id="${key}" value="${key}">${datasetMethodQuery.dataset.queryParams[key].label}</option>`);
            }
            
            // on query select, init a new QueryConstraint and add to list, disable that query from select
            $(`#${datasetCode}-${methodCode}-query-select`).change(function(event) {
                var queryCode = event.currentTarget.value;
                datasetMethodQuery.constraints[queryCode] = addQueryConstraint(
                                                                        datasetMethodQuery.dataset.queryParams[queryCode].type,
                                                                        queryCode, 
                                                                        datasetMethodQuery.dataset.queryParams[queryCode],
                                                                        datasetMethodQuery.datasetCode,
                                                                        datasetMethodQuery.methodCode,
                                                                        datasetMethodQuery
                                                                    );
                
                // disable current query from select
                $(`#${datasetCode}-${methodCode}-query-select option[value=${queryCode}]`).prop('disabled', true);
                // set selected value to 'Add query field...
                $(`#${datasetCode}-${methodCode}-query-select option[value=default]`).prop('selected', true);
                $('#update-query-button').trigger('constraint:change');
                return false;
            });
            
            $(`#${datasetCode}-${methodCode}-remove`).on('click', function(event) {
                /*
                 * Remove current dataset from table
                 * Change dataset select default to 'Add dataset...' and enable all datasets
                 */
                $(`#add-dataset-select option#${datasetCode}-${methodCode}`).prop('disabled', false);
                $(`#add-dataset-select option[value=default]`).html('Add dataset...');
                removeDatasetMethodQuery(datasetCode, methodCode);
            ...
            return datasetMethodQuery;
            
        };
        
        const removeDatasetMethodQuery = (datasetCode, methodCode) => {
              var datasetMethodQueries = queryBuilder.datasetMethodQueries;
              for (let i=datasetMethodQueries.length; i--;) {
                  if (datasetMethodQueries[i].datasetCode === datasetCode &&
                            datasetMethodQueries[i].methodCode === methodCode) {
                      datasetMethodQueries.splice(i,1);
                      break;
                  }
              }
              $(`#${datasetCode}-${methodCode}-query`).remove();
              $(`#${datasetCode}-${methodCode}-spacer`).remove();
        };
        
        const addQueryConstraint = (type, queryCode, queryParams, datasetCode, methodCode, parent) => {
            ...
            $(`#${datasetCode}-${methodCode}-query-constraints-table > tbody`).append(`<tr id="${datasetCode}-${methodCode}-${queryCode}-query" class="query-constraint-row">
                                                                         ...
                                                                         <tr id="${datasetCode}-${methodCode}-${queryCode}-spacer" class="query-constraint-spacer-row">
                                                                            <td></td><td></td><td></td>       
                                                                         </tr>`);
            
            // listener to remove constraint from query
            $(`#${datasetCode}-${methodCode}-${queryCode}-query > #query-remove`).on('click', function(event) {
                if (!queryConstraint.parent.excluded) {
                    ...
                    $(`#${datasetCode}-${methodCode}-query-select option[value=${queryCode}]`).prop('disabled', false);
                    // remove query row and spacer row
                    $(`#${datasetCode}-${methodCode}-${queryCode}-query`).remove();
                    $(`#${datasetCode}-${methodCode}-${queryCode}-spacer`).remove();
                    $('#update-query-button').trigger('constraint:change');
                }
            ...
            const radioConstraint = function() {
                ...
                for (let i=0; i<queryValues.length; i++) {
                    $(`#${datasetCode}-${methodCode}-${queryConstraint.queryCode}-query > #query-control > form`).append(`<input
                                                                                            type="radio" 
            ...
            const checkboxConstraint = function() {
                ...
                $(`#${datasetCode}-${methodCode}-${queryConstraint.queryCode}-query > #query-control`).append('<form>');
                var queryValues = queryConstraint.queryParams.options.values;
                var queryLabels = queryConstraint.queryParams.options.labels;
                for (let i=0; i<queryValues.length; i++) {
                    $(`#${datasetCode}-${methodCode}-${queryConstraint.queryCode}-query > #query-control > form`).append(`<input 
            ...
            success: function(data) {
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    for (let j = 0; j < data[i].methods.length; j++) {
                        ...
                        queryBuilder.data[datasetCode][methodCode] = {name: datasetMethodName};
                        $('#add-dataset-select').append(
        ...
        $('#add-dataset-select').change(function(event) {
             ...
             */
            if (queryBuilder.datasetMethodQueries.length) {
                let oldDatasetCode = queryBuilder.datasetMethodQueries[0].datasetCode;
                let oldMethodCode = queryBuilder.datasetMethodQueries[0].methodCode;
                removeDatasetMethodQuery(oldDatasetCode, oldMethodCode);
                $(`#add-dataset-select option#${oldDatasetCode}-${oldMethodCode}`).prop('disabled', false);
                queryBuilder.datasetMethodQueries = [];
            }
            queryBuilder.datasetMethodQueries.push(addDatasetMethodQuery(
                                                    queryBuilder.datasetTableId,
                                                    datasetCode,
                                                    methodCode,
                                                    queryBuilder.data[datasetCode]
                                                    ));
